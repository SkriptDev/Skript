...
public class EffOpenInventory extends Effect {
	...
	public boolean init(final Expression<?>[] exprs, final int matchedPattern, final Kleenean isDelayed, final ParseResult parseResult) {
		...
		invi = open ? (Expression<Object>) exprs[0] : null;
	...
	protected void execute(final Event e) {
		if (invi != null) {
			final Object i = invi.getSingle(e);
			if (i == null)
				return;
			for (final Player p : players.getArray(e)) {
				try {
					...
					} else if (i instanceof Merchant) {
						p.openMerchant((Merchant) i, true);
					}
	...
	public String toString(final @Nullable Event e, final boolean debug) {
		String inv = this.invi != null ? this.invi.toString(e, debug) : null;
		return this.open ? "open " + inv + " to " + players.toString(e, debug) :
			"close inventory of " + players.toString(e, debug);
	}
