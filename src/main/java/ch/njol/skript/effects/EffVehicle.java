...
public class EffVehicle extends Effect {
	static {
		Skript.registerEffect(EffVehicle.class,
			"(make|let|force) %entities% [to] (ride|mount) [(in|on)] %" + (PassengerUtils.hasMultiplePassenger() ? "entities" : "entity") + "/entitytypes%",
	...
	protected void execute(final Event e) {
		...
		for (final Object v : vs) {
			...
			} else if (v instanceof EntityType entityType) {
				for (final Entity p : ps) {
					assert p != null : passengers;
					final Entity en = p.getWorld().spawnEntity(p.getLocation(), entityType);
					if (en == null)
						return;
