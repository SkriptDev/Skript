...
public abstract class SkriptJUnitTest {
	...
	/** Spawns a testing entity at the spawn location of the testing world.
	 * @param clazz Type of entity to spawn
	 * @param <E> Entity class of entity to spawn
	 * @return Entity which was spawned
	 */
	public static <E extends Entity> E spawnTestEntity(Class<E> clazz) {
		if (delay <= 0D)
			delay = 1; // A single tick allows the piggy to spawn before server shutdown.
		return (E) getTestWorld().spawn(getTestLocation(), clazz);
	}
