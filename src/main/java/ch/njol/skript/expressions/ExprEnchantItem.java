...
@Since("2.5")
public class ExprEnchantItem extends SimpleExpression<ItemStack> {

	static {
		Skript.registerExpression(ExprEnchantItem.class, ItemStack.class, ExpressionType.SIMPLE, "[the] enchant[ed] item");
	}
	...
	protected ItemStack[] get(Event e) {
		if (e instanceof PrepareItemEnchantEvent)
			return new ItemStack[]{((PrepareItemEnchantEvent) e).getItem()};
		else if (e instanceof EnchantItemEvent)
			return new ItemStack[]{((EnchantItemEvent) e).getItem()};
		else
			return null;
	}
		...
		switch (mode) {
			case SET:
				if (event instanceof PrepareItemEnchantEvent) {
					...
					e.getItem().setType(item.getType());
