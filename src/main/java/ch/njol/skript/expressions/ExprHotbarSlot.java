...
@Name("Hotbar Slot")
@Description({"The currently selected hotbar <a href='./classes.html#itemstack'>slot</a>.",
...
@Since("2.2-dev36")
public class ExprHotbarSlot extends PropertyExpression<Player, ItemStack> {

	static {
		registerDefault(ExprHotbarSlot.class, ItemStack.class, "[([current:currently] selected|current:current)] hotbar slot[s]", "players");
	}
	...
	protected ItemStack[] get(Event event, Player[] source) {
		return get(source, new Getter<ItemStack, Player>() {
			@Override
			@Nullable
			public ItemStack get(Player player) {
				...
				if (event instanceof PlayerItemHeldEvent switchEvent && time != EventValues.TIME_NOW) {
					if (time == EventValues.TIME_FUTURE)
						return inventory.getItem(switchEvent.getNewSlot());
					if (time == EventValues.TIME_PAST)
						return inventory.getItem(switchEvent.getPreviousSlot());
				}
