...
@Examples({"send true if entire tooltip of player's tool is shown",
	...
	static {
		if (Skript.methodExists(ItemMeta.class, "isHideTooltip")) {// this method was added in the same version as the additional tooltip item flag
			Skript.registerCondition(CondTooltip.class,
				"[the] [entire|:additional] tool[ ]tip[s] of %itemstacks% (is|are) (:shown|hidden)",
				"[the] [entire|:additional] tool[ ]tip[s] of %itemstacks% (isn't|is not|aren't|are not) (:shown|hidden)",
				"%itemstacks%'[s] [entire|:additional] tool[ ]tip[s] (is|are) (:shown|hidden)",
				"%itemstacks%'[s] [entire|:additional] tool[ ]tip[s] (isn't|is not|aren't|are not) (:shown|hidden)"
			);
		}
	}
