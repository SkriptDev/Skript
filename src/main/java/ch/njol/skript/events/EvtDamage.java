...
public class EvtDamage extends SkriptEvent {
	...
	static {
		Skript.registerEvent("Damage", EvtDamage.class, EntityDamageEvent.class,
				"damag(e|ing) [of %-entitytype%] [by %-entitytype%]")
	...
	@Nullable
	private Literal<EntityType> ofTypes, byTypes;

	@Override
	public boolean init(Literal<?>[] args, int matchedPattern, ParseResult parser) {
		ofTypes = (Literal<EntityType>) args[0];
		byTypes = (Literal<EntityType>) args[1];
		return true;
	}
	...
	private boolean checkDamager(Entity e) {
		if (byTypes != null) {
			for (EntityType d : byTypes.getAll()) {
				if (d == e.getType())
					return true;
			}
			return false;
	...
	private boolean checkDamaged(Entity e) {
		if (ofTypes != null) {
			for (EntityType d : ofTypes.getAll()) {
				if (d == e.getType())
					return true;
			}
			return false;
