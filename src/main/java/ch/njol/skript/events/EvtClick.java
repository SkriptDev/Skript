...
public class EvtClick extends SkriptEvent {
	 ...
	 */
	@Nullable
	private Literal<Material> tools;
	...
	public boolean init(Literal<?>[] args, int matchedPattern, ParseResult parseResult) {
		...
		tools = (Literal<Material>) args[1 - matchedPattern];
		return true;
	}
	...
	public boolean check(Event event) {
		...
		if (tools != null && !tools.check(event, new Checker<Material>() {
			@Override
			public boolean check(final Material t) {
				if (event instanceof PlayerInteractEvent playerInteractEvent) {
					return t == playerInteractEvent.getMaterial();
...
//					} else {
//						return o instanceof EntityData ? false : ((Material) o).isOfType(block);
//					}
					return false; // TODO gotta figure this out
				}
			});
		}
		return true;
